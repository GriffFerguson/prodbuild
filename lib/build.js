...
function build() {
    var files = findFiles(path.normalize(config.entry));
    exec(`rm -rf ${path.normalize(config.output)}*`)
...
function checkExclude(fileName) {
    if (config.exclude) {
        config.exclude.forEach(excludeFile => {
            if (fileName === excludeFile) return true
        })
    } else return false
}