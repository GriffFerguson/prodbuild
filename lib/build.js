...
function build() {
    ...
    for (file of files) {
        ...
        if (file.meta.ext == '.js') {
            ...
            // CSS
            var innerFolder = file.meta.path.split(path.join(config.output, config.entry.styles))[1]
            var filePath = path.join(config.output, config.entry.styles, innerFolder == undefined ? '' : innerFolder)
            filePath.indexOf(undefined) ? path.join(config.output, config.entry.styles) : filePath
            buildDirectory(filePath)
            console.log(__dirname)
            var directory = process.cwd()
            var styleInput = path.resolve(file.meta.path, file.meta.name)
            var styleOutput = path.resolve(filePath)
            exec(`cd ${__dirname} && npx postcss ${styleInput} > ${styleOutput}/${file.meta.name}`)
            console.log(`npx postcss ${styleInput} > ${styleOutput}/${file.meta.name}`)
            exec(`cd ${directory}`)
