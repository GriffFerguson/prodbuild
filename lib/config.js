...
do {
    if (fs.existsSync(join(searchDir,"/build.config.json"))) {
        ...
        if (searchDir == prevDir) {
            isRootDir = true;
            root = process.cwd();
            // log(`Could not find a project root directory due to a lack of a "build.config.json" file. Please run "npx prodbuild init" to generate one. For now, "${root} will be used as the project diretory and default values will be substituted.`, "error");
        } else {
