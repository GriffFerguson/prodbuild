    ...
    if (rc.active && process.argv[2] == 'start') {
        ...
        if (rc.pid != undefined && rc.active === true) {
            try {
                ...
                console.log('Error: Prodbuild encountered an error shutting down the server; attempting to restart the server instead.')
                fs.appendFileSync(
                    '.prodbuildrc/log.txt',
                    `\n[${stamp.date} ${stamp.time}]  SYSTEM ERROR: Failed to stop server, attempting to restart`
                )
                try {
                    fs.rmSync('./.prodbuildrc/cache', {recursive: true})
                    spawnServer()
                    ...
                    throw "Error: Failed to restart server.\nDetailed error for debugging:\n" + error
                }
            }
        } else {
